...
- 页头下三张概览统计卡合并为一行元信息条（detail-status-bar），无数据的项直接不渲染；空值字段整体隐藏策略贯穿基本信息网格。
- “备注”并入基本信息卡底部小节，支持页内添加/编辑（PATCH `/api/schools/[id]/note`），清空后学校自动移出“特别备注院校”清单。
- “最近更新”时间线改为竖线圆点样式，去除条目外框与重复日期；内部备注为限宽琥珀块。

**全局编辑模式（替代整页编辑为主入口）**

- 页头唯一入口“✏️ 编辑模式”切换浏览/编辑态；浏览态零编辑按钮。
- 进入后顶部琥珀提示条（含“N 张卡片有未保存修改”计数），所有卡片自动展开为分区表单：基本信息、合作关系、机密字段（招生计划/考核安排/合作收费）、每个项目各自表单；每卡“保存本区/还原”，退出时脏卡弹确认防丢失。
- 新增接口：`PATCH /api/schools/[id]/basic`、`PATCH /api/schools/[id]/confidential`（仅 ADMIN）、`PATCH /api/programs/[id]`，均带校验+审计+缓存刷新。
- 项目保存逻辑抽取为 `src/lib/program-editor.ts` 共享模块，整页编辑接口（`/api/schools/update`）同步复用，两条编辑路径不会漂移。整页编辑路由仍在（URL 直达），页头入口已移除。

**市场经理视图字段过滤**

- 筛查结果卡片：市场经理仅见学校/项目(含类型)/地区/截止日期与开放状态/专业（按授课语言标注“专业（中授）/（英授）”）/学费/住宿费/奖学金/可招生人数/是否面试（悬停看安排）/招生条件；隐藏匹配结论徽章、证据条、首年上限金额、导师接收函与生源地偏好徽章、授课语言独立项、顾问推荐理由输入框。勾选与查看详情保留。
- 学校库列表：市场经理视图原有隐藏基础上补“备注”列（现有：学校/地区/项目数/备注）；移动端卡片同步。
- 其他角色视图完全不变（实测管理员筛查证据条照常）。
- 分组标题（可直接申请/需要补充条件等）暂保留作导航，是否对市场经理拍平待产品确认。

**合作收费机密字段**

- `schools.cooperation_fee_text`（迁移 `drizzle/0005_school_cooperation_fee.sql`）。
- 权限沿用机密体系：ADMIN 可改，ADMIN/DATA_MANAGER 可看；详情页机密字段新增“合作收费”分组，整页编辑机密区新增输入。
- Excel 导入映射（合作收费列）、维护模板导出表头、手工录入 schema 全链路接入。

**启动自动迁移**

- `src/lib/db/index.ts` 进程内首个连接建立前自动执行未应用迁移（幂等）。任何部署路径都不再依赖手工 `db:migrate`；本次 0005 上线即靠该机制。

**修复**

- basic 接口审计 `changed` 失真：旧值查询改 camelCase 别名，已用对照实验验证精确性（无变更→[]，单字段→[city]）。
- 备注编辑器由受控 state 改非受控 defaultValue+FormData，消除“清空提交却写回旧值”的竞态。

**测试与部署**

- 验证：typecheck、lint 无错误、159 单测通过、覆盖率 75.33%、生产构建通过；新增 listMarketPrograms 已随项目总表方案撤回（相关测试回退）。
- 浏览器端到端：管理员编辑模式全链路（进入/分区保存/脏卡退出确认）、项目卡真实编辑（深圳大学项目学制修改落库、专业完好后还原）、市场经理三页面核对、越权调接口防护（307 拒绝且数据无损）、备注回归——全部通过，测试数据均已还原。
- 部署：master 已推送至 GitHub（5e40eaad）。目标宝塔服务器 https://8.210.20.252:35889/home 入口返回 404（安全入口路径待用户提供）；确定走“宝塔终端粘贴命令”两步部署（先探测环境布局，再执行发布脚本式命令块），尚未开始执行。
...
- 新增 `src/lib/permissions.ts`：统一维护院校编辑/导入/审计/账号管理角色集合、12 个机密字段清单，以及市场经理可见字段清单。
- 院校机密字段（合作与招生信息 12 个字段）仅 `ADMIN` 可查看和修改；学校详情页、编辑页、手动录入、Excel 导入预览、模板导出均按角色过滤，非高级管理员提交机密字段会被后端剥离。
- 市场经理只读，学校列表和详情仅展示校名、地址、专业、申请条件、截止时间、学费、住宿费。
- 渠道资源部可进入数据导入/手动录入并编辑普通院校信息，但不能查看或写入机密字段。
- 验证：`npm run typecheck`、改动文件 `eslint`、`npm run build` 通过；`npm test` 122 通过，1 个既有失败为 `admin/users/route.test.ts` 期望 403 但历史提交已改为 303 重定向，与本轮无关。
