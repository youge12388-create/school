...
6. 筛选时为全部有效项目计算学历、语言、专业、预算、CSCA、成绩、年龄、地区、住宿与截止日期证据，再按可直接申请、需补条件、信息未知和明确不符合分组；已截止项目保留并折叠。
7. 筛选结果通过学校内部 ID 进入学校详情页；详情页按上传 Excel 的原始列顺序展示高校汇总和该校全部项目字段，优先读取每行保留的 `rawJson`，结构化字段仅作缺失回退，不复制或推断业务数据。
8. 推荐方案保存当时的筛选条件和证据，后续打印不会依赖重新计算。
...
- 筛查模块的学校/项目联表读取使用具名列的原生 SQLite 查询，避免 sqlite-proxy 在复杂联表结果中发生字段顺序错位，并保证详情链接始终使用学校内部 ID。
...
- 客户列表按负责老师和签约状态直接过滤；院校录取情况从有效申请状态实时汇总，避免在客户表重复保存可能失真的录取结果。
- 后续跟进摘要读取最近一条 `follow_ups`，完整沟通时间线仍在客户详情页展示。
- 新增客户与对应审计日志使用同一个原生 SQLite 事务，任一步失败都会整体回滚。
- 当前用户会话使用原生 SQLite 具名列查询，避免 sqlite-proxy 在生产联表结果中发生字段错位并触发审计外键失败。
