...
系统当前是本地一体化 Web 应用：Next.js 负责页面和内部 API，SQLite 保存业务数据，上传材料在本机目录中加密存放。支持本地账号和企业微信自建应用登录；不包含外部客户分享、短信、支付或外部 AI 推荐。

企业微信作为部署层之外的独立适配层接入，不改变学校、项目、客户、申请和材料这些核心业务模型。
...
## 目录与职责
    ...
    src/lib/wecom.ts          企业微信 OAuth、通讯录 API 与数据规范化
    src/lib/wecom-service.ts  企业微信组织同步、部门角色映射与账号生命周期
...
- src/lib/db/schema.ts 定义核心表：用户、会话、审计、学校、项目、专业、客户、跟进、推荐、申请、材料、导入批次，以及企业微信部门和成员关系。
...
- 本地账号只能由管理员创建；企业微信账号由组织架构同步或首次成功登录时自动建立。
...
- 院校信息更新（`school_updates`）区分普通更新与机密更新：普通更新对所有角色可见；机密更新、操作人/提交人等人员字段及机密附件仅高级管理员和数据管理员可见，也只有这两类角色可上传/编辑。
- 所有角色当前均可查看全部客户，这是已确认的业务口径。
- 会话 Cookie 名为 school_syt_session，使用 httpOnly、sameSite: strict，生产环境启用 secure。
- 企业微信 Secret 只从服务端环境变量读取；OAuth state 使用短期 httpOnly、sameSite: lax Cookie 校验。
- 企业微信部门与成员同步到独立适配表；管理员按部门配置系统角色。多部门成员取最高优先级角色，未匹配角色默认停用并禁止登录。
- 企业微信账号不能通过密码登录、密码重置或手工修改角色；角色由部门映射和企业微信成员状态控制。角色或状态变化时吊销该账号现有会话。
...
- 企业微信作为登录和组织架构适配层，不改变学校、项目、客户、申请和材料核心业务模型。
