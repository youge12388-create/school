...
2. matchPrograms() 根据客户筛选条件生成匹配结果、证据和排序分数。
3. 专业匹配使用关键词和人工维护同义词，不调用外部 AI。
4. 预算按首年费用上限判断；费用缺失时展示风险提示。
5. GPA 仅在计分制一致时自动比较；无法可靠换算时提示人工复核。
6. 截止日期未过的项目优先；已过期项目保留并折叠；日期未知继续展示并警告。
7. 推荐方案保存当时的筛选条件和证据，后续打印不依赖重新计算。
