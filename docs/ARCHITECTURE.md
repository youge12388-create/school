...
- ADMIN（高级管理员）：全部权限，含院校机密字段的查看与修改、账号和审计管理。
- DATA_MANAGER：顾问权限，加知识库导入和复核；院校机密字段仅高级管理员可见。
- CHANNEL_RESOURCE（渠道资源部）：院校信息录入与更新，不包含机密字段。
- MARKET_MANAGER（市场经理）：只读查看校名、地址、专业、申请条件、截止时间、学费、住宿费。
- ADVISOR：筛选、客户、申请和材料操作。
- 院校机密字段（合作与招生信息 12 个字段）仅 ADMIN 可查看和修改；学校详情、编辑页、导入预览、手动录入、模板导出均按角色过滤。
