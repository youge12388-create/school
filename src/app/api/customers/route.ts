...
export async function POST(request: Request) {
  try {
    ...
    const requestedOwnerId = asText(formData.get("ownerId")) || user.id;
    const owner = sqlite
    ...
    const requestedContractStatus = asText(formData.get("contractStatus"));
    const contractStatus = CONTRACT_STATUSES.includes(
      requestedContractStatus as ContractStatus,
    )
      ? (requestedContractStatus as ContractStatus)
      : "UNKNOWN";
    const id = newId();
    ...
    try {
      sqlite.prepare(`INSERT INTO customers (
        ...
        accommodation_required, date_of_birth, owner_id, contract_status, notes,
      ...
      await writeAudit(
        {
          ...
          entityType: "CUSTOMER",
          entityId: id,
          details: { customerNo, name, ownerId: owner.id, contractStatus },
        },
        sqlite,
      );
      sqlite.exec("COMMIT");
